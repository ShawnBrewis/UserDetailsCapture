...
namespace UserDetailsCapture.Controllers
{
    ...
    public class HomeController : Controller
    {
            ...
            if (Request.Params["id"] != null)
            {
                ...
                foreach (var row in data)
                {
                    try
                    {
                        ...
                        users.Day = int.Parse(dateString[1]);
                        users.Month = int.Parse(dateString[0]);
                        users.Year = int.Parse(dateString[2]);
                    }
                    catch (Exception T)
                    {
        ...
        {
            ViewBag.Message = "Email Capture Form";
            return View(new Models.EmailModel());
        }
